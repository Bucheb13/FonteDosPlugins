...
export default function PaginaAdminEditarDaw() {
  ...
  const carregar = useCallback(async () => {
    ...
    const res = await fetch(
      `/api/admin/daws/obter?slug=${encodeURIComponent(slug)}`,
      {
        headers: { "x-senha-admin": senhaAdmin },
        cache: "no-store",
      }
  ...
  const salvar = useCallback(async () => {
    ...
    // Se quiser permitir texto vazio quando tipo=texto, remova esse if
  ...
  if (!senhaAdmin)
    return (
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 md:p-8 text-sm text-white/70">
        Digite a senha do admin no header para editar este daw.
      </div>
    );
  if (validando)
    return (
  ...
  if (carregando || !daw)
    return (
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 md:p-8 text-sm text-white/70">
        {carregando ? "Carregando…" : "Daw não encontrado."}
      </div>
    );
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/5 p-6 md:p-8">
        ...
        <p className="mt-1 text-sm text-white/60">
          Atualize a capa e/ou o .torrent deste daw (1600×900).
        </p>
