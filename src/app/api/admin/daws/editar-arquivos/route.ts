...
export async function POST(req: Request) {
  ...
  await registrarAuditoriaAdmin(req, { acao: "POST", entidade: "daws/editar-arquivos" });
  ...
  const { data: dawAtual, error: erroBusca } = await supabase
    .from("daws")
  ...
  if (erroBusca) {
    return jsonErro("Erro ao buscar DAW atual.", 500);
  }
  
  if (!dawAtual) {
    return jsonErro("DAW não encontrada.", 404);
  }
...
if (capa instanceof File) {
  ...
  if (dawAtual.imagem_capa_url) {
    const base = publicBase.replace(/\/$/, "");

    if (dawAtual.imagem_capa_url.startsWith(base)) {
      const keyAntiga = dawAtual.imagem_capa_url.replace(`${base}/`, "");
