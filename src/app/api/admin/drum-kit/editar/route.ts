...
function validarCategoria(v: unknown): CategoriaDrumKit | null {
...
export async function POST(req: Request) {
  ...
  await registrarAuditoriaAdmin(req, { acao: "POST", entidade: "drum-kit/editar" });
  ...
  if (typeof b.categoria !== "undefined") {
    ...
    if (!cat) {
      return NextResponse.json(
        { erro: "Categoria inválida (use: drum-kit | sample-kit | midi-kit)." },
        { status: 400 }
      );
    }
    patch.categoria = cat;
  ...
  if (tipoNoPatch || conteudoNoPatch) {
    ...
    // se conteúdo veio vazio/null -> zera ambos
    if (!conteudo) {
      patch.conteudo_instalacao = null;
