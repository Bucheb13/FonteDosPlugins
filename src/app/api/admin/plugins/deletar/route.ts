...
export async function POST(req: Request) {
  ...
  await registrarAuditoriaAdmin(req, { acao: "POST", entidade: "plugins/deletar" });
  ...
  // 🔹 Buscar plugin
  const { data: plugin, error: erroBusca } = await supabase
    .from("plugins")
  ...
  // 🔹 Deletar TORRENT
  if (plugin.r2_chave_arquivo) {
    await r2.send(
      new DeleteObjectCommand({
        Bucket: bucket,
        Key: plugin.r2_chave_arquivo,
      })
    );
  }

  // 🔹 Deletar CAPA
  if (plugin.imagem_capa_url) {
    const base = process.env.NEXT_PUBLIC_R2_PUBLIC_BASE_URL?.replace(/\/$/, "");
    if (base && plugin.imagem_capa_url.startsWith(base)) {
      const key = plugin.imagem_capa_url.replace(`${base}/`, "");
