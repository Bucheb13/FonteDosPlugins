...
function validarCategoria(v: unknown): Categoria | null {
...
export async function POST(req: Request) {
  ...
  await registrarAuditoriaAdmin(req, { acao: "POST", entidade: "plugins/editar" });
  ...
  // ✅ categoria
  if (typeof b.categoria !== "undefined") {
    const cat = validarCategoria(b.categoria);
    if (!cat) {
      return NextResponse.json(
        { erro: "Categoria inválida (use: efeitos | instrumentais)." },
        { status: 400 }
      );
    }
    patch.categoria = cat;
  ...
  // ===============================
  const tipoNoPatch =
    Object.prototype.hasOwnProperty.call(patch, "tipo_instalacao");
  const conteudoNoPatch =
    Object.prototype.hasOwnProperty.call(patch, "conteudo_instalacao");
