...
export async function POST(req: Request) {
  ...
  await registrarAuditoriaAdmin(req, { acao: "POST", entidade: "programas/editar-arquivos" });
  ...
  const { data: programaAtual, error: erroBusca } = await supabase
    .from("programas")
  ...
  if (erroBusca) {
    return jsonErro("Erro ao buscar programa atual.", 500);
  }
  
  if (!programaAtual) {
    return jsonErro("Programa não encontrado.", 404);
  }
  ...
  if (torrent instanceof File) {
    ...
    const chaveTorrent = `programas/torrents/${slug}.torrent`;
...
if (capa instanceof File) {
  ...
  if (programaAtual.imagem_capa_url) {
    const base = publicBase.replace(/\/$/, "");

    if (programaAtual.imagem_capa_url.startsWith(base)) {
      const keyAntiga = programaAtual.imagem_capa_url.replace(`${base}/`, "");
  ...
  // ✅ UPLOAD DA NOVA CAPA
  const chaveCapa = `programas/capas/${slug}.${ext}`;
