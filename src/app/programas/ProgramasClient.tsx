...
export default function ProgramasClient({
  ...
  const buscarProgramas = useCallback(async (query: string) => {
    setCarregando(true);
    setErro("");

    try {
      const url = new URL("/api/programas", window.location.origin);
      ...
      if (!res.ok) {
        setErro(data?.erro ?? "Erro ao carregar Programas");
        setProgramas([]);
        return;
      }
  ...
  useEffect(() => {
    void buscarProgramas(debouncedPesquisa);
  }, [debouncedPesquisa, buscarProgramas]);

  const placeholder = "Busque por programas...";
