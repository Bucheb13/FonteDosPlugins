...
export default async function TopBaixadosSemanaPlugins({
  limite = 12,
  className = "",
  dados,
}: Props) {
  ...
  // ✅ somente plugins
  const rows = (data as RowTop[]).filter((r) => r.item_tipo === "plugin");
  ...
  const cards: CardData[] = rows
    .map<CardData | null>((r, index) => {
      const item = mPlugins.get(r.item_id);
      if (!item) return null;
          ...
          <div className="relative p-6 md:p-8">
            <div className="mb-6 flex justify-center">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="flex">
  <PureCss3D
    text={"Plugins Mais Baixados"}
    className="
-translate-x-18 scale-[0.54]
