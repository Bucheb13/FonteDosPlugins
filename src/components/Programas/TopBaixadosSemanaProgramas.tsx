...
export default async function TopBaixadosSemanaDaws({
  limite = 12,
  className = "",
  dados,
}: Props) {
  ...
  const rows = (data as RowTop[]).filter((r) => r.item_tipo === "programa");
  if (!rows.length) return null;

  const mDaws = new Map(dados.programas.map((i) => [i.id, i]));

  const cards: CardData[] = rows
    .map<CardData | null>((r, index) => {
      const item = mDaws.get(r.item_id);
      if (!item) return null;
          ...
          <div className="relative p-6 md:p-8">
            <div className="mb-6 flex justify-center">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="flex">
                  <PureCss3D
                    text={"Programas Mais Baixados"}
                    className="
                    -translate-x-21 scale-[0.50]
